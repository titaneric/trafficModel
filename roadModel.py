...
def determineCoords(event, line_distance):
    ...
    index = itemID[0]
    x, y = (index // (canvas_height // line_distance), index % (canvas_height // line_distance) - 1)
    return x, y
...
#canvas.bind("<B1-Motion>", lambda event, line_distance = distance: createRoad(event, line_distance))
