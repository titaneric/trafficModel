from lane_position import LanePosition
import sys
import math
...
class Trajectory():
    ...
    def absolutePosition(self):
        return self.temp.position if self.temp.lane is not None else self.current.position
 
    @property 
    def relativePosition(self):
        return absolutePosition() // self.lane.length

    @property
    def direction(self):
        return self.lane.getDirection(relativePosition())

    @property
    def coords(self):
        return self.lane.getPoint(relativePosition())
    ...
    def getDistanceToIntersection(self):
        distance = self.current.lane.length - self.car.length // 2 - self.current.position
        return max(distance, 0) if not self.isChangingLanes else math.inf
