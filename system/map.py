...
interID_list = ["intersection_" + str(num + 1)
                for num in range(intersectionNum)]
jsonDict = dict()
intersectionDict = dict()
crossDict = dict()
for i in range(S):
    for j in range(S):
        pos = {"x": i * roadLen, "y": j * roadLen,
               "height": interHi, "width": interWid}
        intersectionDict[interID_list[S * i + j]
                         ] = {"position": pos, "id": interID_list[S * i + j]}
        # deal with the inner intersection
        if i > 0 and i < (S - 1) and j > 0 and j < (S - 1):
            linkList = [interID_list[(i - 1) * S + j], interID_list[(i + 1) * S + (
                j)], interID_list[(i) * S + (j - 1)], interID_list[i * S + (j + 1)]]
            crossDict[interID_list[i * S + j]] = sorted(linkList)

        if i > 0 and i < (S - 1) and j == 0:  # deal with the left boundary
            linkList = [interID_list[(
                i - 1) * S + j], interID_list[(i + 1) * S + j], interID_list[i * S + (j + 1)]]
            crossDict[interID_list[i * S + j]] = sorted(linkList)
        ...
        if i == 0 and j > 0 and j < (S - 1):  # deal with upper boundary
            linkList = [
                interID_list[i * S + (j - 1)], interID_list[i * S + (j + 1)], interID_list[(i + 1) * S + j]]
            linkList = sorted(linkList)
            crossDict[interID_list[i * S + j]] = linkList
        ...
        if i == 0 and j == 0:  # deal with the upper left corner
            linkList = [interID_list[i * S +
                                     (j + 1)], interID_list[(i + 1) * S + j]]
            linkList = sorted(linkList)
            crossDict[interID_list[i * S + j]] = linkList
...
for inter, connect in crossDict.items():
    for subConnect in connect:
        roadDict["road_" + str(cnt)] = {"source": inter,
                                        "id": "road_" + str(cnt), "target": subConnect}
        cnt += 1
